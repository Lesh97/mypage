let popupProps = [
  {
    ...
    thinking: `
      ...
      이 프로젝트를 진행하면서 가장 힘들었던 부분은 CORS 문제 해결입니다. OTT서비스처럼 서버를 구축해서 해결했지만 처음에는 프론트단에서 어떻게든 해결을 할 수 있지않을까? 라는 생각으로 다가갔습니다.
      하지만 다양한 방법이 존재하지만, 결국 포기한 이유는 안정성이 보장되지 않는다는 생각이 들었기때문에 결국 Access-Control-Allow-Origin으로 해결을 했습니다. 
      `,
  ...
  },
  {
    ...
    구현을 하면서 가장 애를 먹었던 것은 recoil의 이해였다. 외부API에서 데이터를 받아오고, 다양한 데이터를 Props로 넘겨야 하는데 recoil의 이해가 부족했던터라 recoil방식이 아닌 리액트방식으로 전달을 많이 해서 애를 많이 썼습니다.
...

  {
    ...
    thinking: `트렐로라는 사이트를 처음 알게 되면서 기능이 굉장히 신기하고, 
      ...
      리액트에서 배열의 복사에 대해 다시한번 복습할 수 있는 좋은 기회였다고 생각합니다.
      `,
  ...
  },
  {
    ...
    thinking: `개발을 시작한 후 CSS,JS를 배우고 처음으로 만들어본 투두리스트 서비스를 제공하는 사이트입니다.
      ...
      고민 : 이거 투두리스트 관련해서 넣을거 더 있는지 생각해보기
      `,
